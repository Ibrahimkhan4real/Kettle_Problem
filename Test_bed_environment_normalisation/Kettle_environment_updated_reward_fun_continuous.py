...
class SimpleKettleEnv(gym.Env):
    ...
    def step(self, action):
        state = self._get_obs()
                ...
                # Map normalized action [-1, 1] → [0, 3000]
        normalized_action = np.clip(action, -1.0, 1.0)[0]
        power = self.power_min + (normalized_action + 1.0) * 0.5 * (self.power_max - self.power_min)
