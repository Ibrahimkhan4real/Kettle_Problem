...
scales = [1, 5, 10]
total_timesteps = 2000_000  # reduce for faster runs while comparing
...
for scale in scales:
    ...
    boxplot_data[scale] = rewards_eval
    
    # Debug: print to check variability
    print(f"[Evaluation Rewards] Scale {scale}: {rewards_eval[:10]}")
    print(f"[Evaluation Lengths] Scale {scale}: {lengths_eval[:10]}")
