...
class KettleEnv(gym.Env):
    ...
    This version implements potential-based reward shaping following the theoretical
    framework from "Policy Invariance Under Reward Transformations: Theory and 
    Application to Reward Shaping" (Ng, Harada, Russell, 1999).
    
    Key Theoretical Guarantee:
    If R(s,a,s') is the original reward function and Φ:S→ℝ is any real-valued 
    function on states (potential function), then the transformed reward function:
    
    R'(s,a,s') = R(s,a,s') + F(s,a,s')
    
    where F(s,a,s') = γΦ(s') - Φ(s)
    
    has the same optimal policies as R (for all γ ∈ [0,1)).
    
    Goal: Reach 100°C (±2.5°C) at step 200 (±15 steps) with minimal energy use.
    """
    ...
    def __init__(
        ...
        enable_reward_shaping=True,      # Toggle reward shaping on/off
        potential_scale=100.0,           # Overall scale of potential function
        temp_potential_weight=1.0,       # Weight for temperature component
        time_potential_weight=0.5,       # Weight for time component
        energy_potential_weight=0.2,     # Weight for energy efficiency component
    ):
    ...
    def potential_function(self, temp, step, energy_used_J):
        """
        Compute the potential function Φ(s) for the current state.
        
        This function is designed to guide the agent toward energy-efficient behavior
        while maintaining policy invariance. The choice of potential function affects
        learning speed but not the optimal policy.
        
        Design principles:
        1. Should be higher for states closer to the goal
        2. Should encourage waiting until the right time to heat
        3. Should penalize excessive energy use
        4. Must be bounded to ensure convergence
        """
        # Normalize inputs for stable computation
        temp_normalized = (temp - self.initial_temp) / (self.T_target - self.initial_temp)
        temp_normalized = np.clip(temp_normalized, -0.5, 1.5)  # Allow some overshoot
        
        steps_remaining = self.target_deadline_step - step
        steps_remaining_normalized = steps_remaining / self.target_deadline_step
        
        energy_normalized = energy_used_J / max(1.0, self.theoretical_min_energy)
        
        # 1. Temperature component: Reward progress toward target, but only when timing is appropriate
        if steps_remaining > self.approx_heating_time + 20:
            # Too early - no reward for heating
            temp_potential = 0.0
        elif steps_remaining > 0:
            # Within reasonable heating window
            if abs(temp - self.T_target) <= self.T_acceptable_dev:
                # At target - maximum temperature potential
                temp_potential = self.temp_potential_weight
            else:
                # Approaching target - smooth increase
                temp_achievement = max(0, 1 - abs(temp - self.T_target) / (self.T_target - self.initial_temp))
                temp_potential = self.temp_potential_weight * temp_achievement
                
                # Add urgency factor as deadline approaches
                urgency = max(0, 1 - steps_remaining / self.approx_heating_time)
                temp_potential *= (1 + urgency)
        else:
            # Past deadline
            if abs(temp - self.T_target) <= self.T_acceptable_dev:
                # Achieved goal (even if late)
                temp_potential = self.temp_potential_weight * 0.8
            else:
                # Failed to achieve goal
                temp_potential = -self.temp_potential_weight * 0.5
        
        # 2. Time component: Create urgency as deadline approaches
        if steps_remaining > self.approx_heating_time:
            # Still have plenty of time
            time_potential = 0.0
        elif steps_remaining > 0:
            # Time pressure increases
            time_potential = self.time_potential_weight * (1 - steps_remaining_normalized)
        else:
            # Past deadline
            time_potential = self.time_potential_weight
        
        # 3. Energy efficiency component: Always penalize energy waste
        # This creates a gradient toward energy-efficient solutions
        energy_penalty = -self.energy_potential_weight * energy_normalized
        
        # Combine components with overall scale
        total_potential = self.potential_scale * (
            temp_potential + time_potential + energy_penalty
        )
        
        # Ensure potential is bounded (important for convergence)
        total_potential = np.clip(total_potential, -1000, 1000)
        
        return total_potential

    def calculate_base_reward(self, state_before, state_after, action):
        """
        Calculate the base (original) reward R(s,a,s').
        
        This is the sparse reward function that defines the actual task:
        1. Large positive reward for achieving the goal
        2. Small negative reward for energy consumption
        3. Efficiency bonus when goal is achieved
        """
    ...
    def calculate_shaping_reward(self, state_before, state_after):
        """
        Calculate the shaping reward F(s,a,s') = γΦ(s') - Φ(s).
        
        This is the key formula that guarantees policy invariance.
        The shaping reward provides dense feedback without changing optimal policies.
        """
        ...
        # Policy-invariant shaping reward
