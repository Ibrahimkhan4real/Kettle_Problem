...
from kettle_dynamic_env_v14 import KettleEnv
...
UCB_C = 1.5  # Exploration constant for UCB1. sqrt(2) is common, can be tuned.
MCTS_ITERATIONS_PER_ACTION = 4000  # Number of MCTS simulations (REDUCED from 30000)
...
def get_action_via_mcts(mcts_root_node: Node, num_iterations: int):
    ...
    print(f"MCTS Root (Temp: {mcts_root_node.observation[0]:.2f}, EnvStep: {mcts_root_node.observation[3]:.0f}) Child Stats:")
...
def run_mcts_controlled_episodes():
    ...
    for episode_idx in range(EPISODES_TO_RUN):
        # --- Environment Setup (Aligned with simplified KettleEnv v3) ---
        # ** IMPORTANT **: Assumes `kettle_dynamic_env_v13.py` provides an env
        # compatible with these simplified __init__ parameters.
        episode_initial_temp = 15.0
        episode_ambient_temp = 20.0
        # The simplified env uses target_deadline_step and max_steps from its __init__
        # For this MCTS runner, let's define them here if the env doesn't have fixed ones easily accessible
        # Or, ensure your KettleEnv has these as properties if they are dynamic.
        # Assuming the KettleEnv will use its defaults or you set them here.
        env_max_steps_for_gym = 600 # How long the gym environment itself can run
        
        kettle_env = KettleEnv(
            initial_temp=episode_initial_temp,
            ambient_temp=episode_ambient_temp,
            # max_steps and target_deadline_step should be set in your KettleEnv's __init__
            # If they are not, you might need to pass them here if your env version allows.
            # For v3_simplified_mcts, these are init params.
            # max_steps = env_max_steps_for_gym 
        )
        # Access properties for logging if needed (ensure they exist on your env)
        # T_target = kettle_env.T_target # Assuming T_target is an attribute
        # target_deadline_step = kettle_env.target_deadline_step


        current_obs, _ = kettle_env.reset() # Simplified reset, no options
        ...
        mcts_current_root = Node(deepcopy(kettle_env), False, None, current_obs, action_idx_leading_to_node=None)

        print(f"\n{'='*60}")
        print(f"Starting Episode {episode_idx+1}/{EPISODES_TO_RUN}")
        print(f"{'='*60}")
        # Safely try to access T_target, or use a default
        ...
        for timestep_num in range(MAX_TIMESTEPS_PER_EPISODE):
            ...
            print(f"Kettle Temp: {current_obs[0]:.2f}°C (Step from obs: {current_obs[3]:.0f})")


            mcts_current_root, chosen_action_index = get_action_via_mcts(mcts_current_root, MCTS_ITERATIONS_PER_ACTION)

            if chosen_action_index is None:
                print(f"MCTS did not return a valid action at timestep {timestep_num+1}. Ending episode.")
                if mcts_current_root and mcts_current_root.is_terminal:
                    print("  Reason: MCTS root node indicates a terminal state.")
                break

            action_power_W = kettle_env.actions[chosen_action_index] # Assumes .actions exists
            ...
            if mcts_current_root:
                mcts_current_root.is_terminal = is_done_from_env or is_truncated_from_env
            else: # Should not happen if action was chosen
                print("Error: mcts_current_root is None after action selection. Creating new root.")
                mcts_current_root = Node(deepcopy(kettle_env), is_done_from_env or is_truncated_from_env, None, current_obs, None)
        ...
        # Safely access T_target for logging
        ...
        # Safely access deadline step for logging
        ...
        # Ensure kettle_env.actions is accessible
        try: action_map = kettle_env.actions
        except AttributeError: action_map = {i: f"idx_{i}" for i in range(kettle_env.action_space.n)} # Fallback
