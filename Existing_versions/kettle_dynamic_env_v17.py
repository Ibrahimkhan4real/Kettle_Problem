...
class KettleEnv(gym.Env):
    ...
    def __init__(
        ...
        # Observation space: [temp_diff, water_volume, ambient_temp, steps_diff]
        # FIXED: Defined correct bounds for the observation space variables.
        # temp_diff can be negative (e.g., 20C is -80 from target)
        ...
        # FIXED: Removed calculation of diffs from __init__. They must be calculated in step/reset.
    ...
    def calculate_reward(self, T_next, power_W):
        ...
        # --- Hyperparameters for the Efficiency Mission ---
        # The penalty for using power at any given step. This is the "cost of action".
        PER_STEP_POWER_COST_WEIGHT = 0.01

        # The one-time bonus for succeeding. This must be large enough to be
        # worth pursuing, despite the accumulated energy costs.
        SUCCESS_BONUS = 100.0
        
        # The penalty for failing to meet the temperature target at the deadline.
        FAILURE_PENALTY = -50.0
        
        # --- Reward Calculation ---

        # 1. The Per-Step Reward: A constant penalty for using energy.
        max_power = 3000 # Use a fixed max power for normalization consistency
        normalized_power = power_W / max_power if max_power > 0 else 0
        
        reward = - (PER_STEP_POWER_COST_WEIGHT * normalized_power)

        # 2. The Terminal Reward: The Pass/Fail Final Exam.
        # This is only awarded at the exact deadline step.
        if self.steps == self.target_deadline_step:
            final_temp_error = abs(T_next - self.T_target)
            
            if final_temp_error <= self.T_acceptable_dev:
                # Task Fulfilled: Give the huge success bonus.
                reward += SUCCESS_BONUS
            else:
                # Task Failed: Give a massive penalty.
