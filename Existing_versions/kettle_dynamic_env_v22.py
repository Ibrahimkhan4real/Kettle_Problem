...
class KettleEnv(gym.Env):
    ...
    def calculate_reward(self, T_after_action_1dp, T_before_action_1dp, power_W):
        ...
        self.PER_STEP_POWER_COST_WEIGHT = 0.001   # Keep these significant for efficiency
        self.TEMP_AREA_PENALTY_WEIGHT = 0.005  # Keep these significant for JIT
        
        self.EFFICIENT_IDLING_BONUS = 0.42     # Calibrated for the above area penalty
        
        # MODIFIED: Dynamic Temperature Progress Weight
        self.MIN_TEMP_PROGRESS_WEIGHT = 0.05   # Small progress reward when far from deadline
        self.MAX_TEMP_PROGRESS_WEIGHT = 0.80   # Large progress reward when close to/at deadline
        
        self.OVERSHOOT_PENALTY_WEIGHT = 0.05
        
        # Tolerances (ensure these are correctly set from your previous request)
        self.T_acceptable_dev = 2.0
        self.time_tolerance_seconds = 5.0
        self.time_tolerance_steps = int(round(self.time_tolerance_seconds / self.dt))
        self.goal_achieved_this_episode = False 
        ...
        # 1. Per-Step Power Consumption Cost (scaled by dt)
        ...
        reward -= self.PER_STEP_POWER_COST_WEIGHT * normalized_power * self.dt 

        # 3. Temperature Progress Shaping Reward (Dynamically Scaled)
        # Urgency factor: 0 when steps = 0, approaches 1 as steps approach target_deadline_step
        # We add 1 to self.steps to avoid division by zero if target_deadline_step can be 0 (though unlikely here)
        # and to make the factor ramp up correctly.
        urgency_factor = 0.0
        if self.target_deadline_step > 0: # Avoid division by zero
            urgency_factor = min(1.0, (self.steps + 1.0) / self.target_deadline_step)
        
        effective_temp_progress_weight = self.MIN_TEMP_PROGRESS_WEIGHT + \
                                       (self.MAX_TEMP_PROGRESS_WEIGHT - self.MIN_TEMP_PROGRESS_WEIGHT) * urgency_factor
        
        previous_error_abs = abs(T_before_action_1dp - self.T_target)
        current_error_abs_for_progress = abs(T_after_action_1dp - self.T_target) # Use current_error_abs defined above
        error_delta = previous_error_abs - current_error_abs_for_progress
        reward += effective_temp_progress_weight * error_delta 
        ...
        # 6. Deadline Window Achievement Reward & End-of-Episode Failure Penalty
        if not self.goal_achieved_this_episode:
            deadline_window_start = self.target_deadline_step - self.time_tolerance_steps
            deadline_window_end = self.target_deadline_step + self.time_tolerance_steps
            if deadline_window_start <= self.steps <= deadline_window_end:
