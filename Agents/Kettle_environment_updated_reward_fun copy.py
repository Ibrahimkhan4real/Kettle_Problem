...
class SimpleKettleEnv(gym.Env):
    ...
    def reward_function(self, state ,action ,next_state):
        ...
        current_temp, target_temp, ambient_temp, steps_remaining, total_energy_used = state
        next_temp, _, _, next_steps_remaining, _ = next_state
        ...
        if abs(self.step_count - self.target_step) <= self.time_tolerance:
            reward += -self.beta * max(0, abs(target_temp - next_temp)) # Temperature penalty at target time
            done = True
